...
.territorio-prototipo{--territorio-pitch:3deg;--territorio-giro:-1deg;--territorio-espessura:.22rem;--territorio-sombra:.7rem;display:grid;gap:2rem;padding-block:clamp(3rem,7vw,6rem);border-top:1px solid var(--color-borda)}
.territorio-prototipo[data-profundidade="moderada"]{--territorio-pitch:7deg;--territorio-giro:-1.75deg;--territorio-espessura:.46rem;--territorio-sombra:1.1rem}
...
.territorio-prototipo__moldura::after{position:absolute;z-index:-1;right:9%;bottom:8%;left:12%;height:12%;content:"";background:color-mix(in srgb,var(--color-mata) 20%,transparent);filter:blur(1.25rem);transform:skewX(-12deg);opacity:.55}
.territorio-prototipo__svg{display:block;width:100%;height:auto;overflow:visible;transform-origin:50% 58%;transform:perspective(70rem) rotateX(var(--territorio-pitch)) rotateZ(var(--territorio-giro)) translateY(calc(var(--territorio-espessura) * -1));filter:drop-shadow(0 var(--territorio-espessura) 0 var(--color-mata)) drop-shadow(0 var(--territorio-sombra) .7rem color-mix(in srgb,var(--color-carvao) 22%,transparent));transition:transform var(--duracao-painel) var(--easing-padrao),filter var(--duracao-painel) var(--easing-padrao)}
