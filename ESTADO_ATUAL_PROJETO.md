...
## 2026-09-17 — prazo estendido e retomada da função de divulgação

Decisão do responsável nesta data, registrada em
[`ADR-019`](./docs/decisoes/ADR-019-prazo-estendido-e-funcao-de-divulgacao.md):

1. **O prazo do site foi estendido até 22/09/2026.** O congelamento interno de
   14/09 está superado. As duas datas convergiram, e **não há mais folga** entre
   o site pronto e o envio do pacote à FUNCAP.
2. **O site é peça de divulgação** da pesquisa, do Observatório e do PodObservar
   — e não apenas repositório permanente e prova documental. As duas funções
   coexistem.

O `docs/01-arquitetura-informacao.md` §0.1 afirmava o contrário, e essa frase é a
justificativa explícita, no próprio documento, para subordinar apresentação a
rastreabilidade. Doc 01 §0 e §7 receberam apontamento para a ADR-019;
`PLANO_EXECUCAO_OBSERVATORIO.md` §1 foi atualizado.

### O que muda de prioridade

| Antes | Agora |
|---|---|
| O3, O4 e O5 (divulgação) subordinados a O1 e O2 | divulgação é objetivo de primeira ordem, ao lado da rastreabilidade |
| peso da Home acima da meta: dívida tolerável | desvio a corrigir — a Home é a primeira tela de quem chega |
| `/podobservar` como stub aceitável | produto mais divulgável do projeto, e o mais incompleto no site |

**Nada foi flexibilizado** no fail-closed do plano §4, na proibição de dado
fictício ou na exigência de transcrição vinculada a áudio. Divulgação não
autoriza preencher lacuna com conteúdo plausível.

### Bloqueio de conteúdo do PodObservar

`/podobservar` é stub de rota de 27 linhas. `src/componentes/home/conteudo.ts`
registra 3 episódios publicados no Spotify e no YouTube, com **título, duração,
URL e transcrição `null` nos três** — informação do responsável em 2026-09-13,
sem os metadados no repositório.

A página não pode ser construída além da estrutura enquanto esses dados não
existirem. O bloqueio é de conteúdo, não de código.

Estado: **premissa de divulgação registrada; 5 dias até 22/09; sem push, sem
Preview, sem Production, sem alteração de `main`.**

---

## 2026-09-20 — auditoria do A11, Prestação de Contas e créditos institucionais

### A auditoria do A11 fechou sem exposição incompatível

`src/dados/indicadores/derivados.ts` declarava a unidade documental de origem
como **RESTRITA como conjunto** enquanto o Acervo publicava o workbook integral
com o mesmo SHA-256 e os dezoito derivados por aba. A auditoria somente-leitura
mostrou que a **divergência era documental**: o comentário é de 2026-09-10 e
ficou para trás da decisão humana de 2026-09-16, que autorizou a publicação dos
materiais da pesquisa e revogou expressamente as classificações anteriores.

A autorização está registrada em três lugares independentes — este documento,
`docs/carga/PUBLICACAO_ACERVO_2026-09-16.md` e
`src/dados/classificacao-documental.ts`, que traz
`A11: PUBLICAVEL / concluida`. O CHECK de fail-closed da migração 0004 prova
que a promoção não pôde ocorrer por omissão: `PUBLICAVEL` sem revisão de
privacidade concluída é recusado pelo banco.

Matriz completa por artefato, com natureza, presença de dado identificável e
classificação vigente:
[`docs/auditorias/AUDITORIA_A11_PUBLICACAO_2026-09-20.md`](./docs/auditorias/AUDITORIA_A11_PUBLICACAO_2026-09-20.md).

**Nada foi despublicado, apagado ou reclassificado.** Só o comentário do código
e o docblock do teste correspondente foram corrigidos; nenhuma asserção de
privacidade foi removida.

**Registrado para decisão humana, sem ação técnica:** três artefatos — `A11-06`
(ranking nominal de trabalhadores), `A11-07` (localidades) e `A11-08`
(fornecedores) — passam pelo critério literal da decisão de 2026-09-16, que
trata CPF, telefone e assinatura, mas carregam risco de **reidentificação por
cruzamento**, que é de outra ordem. A remediação mínima está descrita na §5 da
auditoria e **não foi executada**.

`/dados` foi conferida e **não teve nada enfraquecido**: ela serve só agregados,
não renderiza `procedencia` e resolve seus links por rótulo exato contra
`vw_anexo_publico`.

### `/prestacao-de-contas` deixou de ser só a tabela

A rota era a tabela de anexos e três links. Passou a abrir com identificação do
projeto — executor, fomento, linha do edital, quem recebe a prestação, período
de coleta —, seguida das entregas, do acervo por natureza de documento, das
pendências declaradas, das evidências com hash e dos créditos.

**Nenhuma contagem é escrita.** Documentos, arquivos, indicadores, lugares de
campo, municípios e episódios são derivados de `vw_anexo_publico`, do dataset de
indicadores, da tabela de materiais por lugar, do recorte territorial e da view
de episódios. Contagem ausente vira ficha **sem medida**, nunca "0".

A página **não afirma** situação financeira, valor, saldo, parecer, aprovação
nem prestação encerrada — nada disso está documentalmente confirmado —, e diz
isso em voz alta. O Caderno de Estudos aparece como pendência declarada, sem
link provisório e sem data estimada, fechando a lacuna que o registro do
primeiro deployment apontara.

### Créditos institucionais: o crédito textual saiu da pendência

O bloco de créditos de fomento estava vazio desde a fundação do layout, à
espera do manual de aplicação de marcas (item E02, ainda `PENDENTE`). A
auditoria do corpus mostrou que o manual governa a **aplicação gráfica** —
proporção, ordem visual, área de reserva —, não quem financia o projeto.

`src/componentes/institucional/creditos.ts` passa a declarar a régua na ordem
normativa da decisão humana registrada na tarefa 16 §2, item 5:

```
Realização → FUNCAP + Governo de Sergipe → PNAB → separador → MinC + Governo Federal
```

`ReguaDeCreditos` a renderiza como lista ordenada — a ordem é conteúdo — no
rodapé compartilhado e na Prestação de Contas. **Nenhuma marca oficial foi
aplicada**: os 23 PNG existem fora do Git, nenhum derivado web foi produzido, e
o manual da PNAB e o da FUNCAP continuam não localizados. A pendência é
declarada na própria superfície.

**Depende de validação técnica ("nada a opor") antes de fechar:** a aplicação
gráfica das marcas — proporção, área de não interferência, largura mínima e a
regra do selo de programa de governo.

### Validações

`pnpm verificar` completo, verde. 928 testes unitários (25 novos em
`testes/prestacao-de-contas.test.ts`) e 302 de acessibilidade (9 novos em
`testes/a11y/prestacao-axe.spec.ts`, incluindo axe em claro e escuro nas duas
rotas da Sala).

Orçamento da Home preservado nos dois perfis bloqueantes: **desktop DPR1
484.641 B** e **mobile DPR2 446.011 B**, contra o teto de 500.000 B. Desktop
DPR2, que é só observabilidade, ficou em 501.940 B.

Estado: **auditoria do A11 fechada sem exposição incompatível; Prestação de
Contas reorganizada; crédito textual de fomento publicado, marcas ainda
pendentes; sem Preview, sem Production, sem alteração de `main`.**
