...
describe("fundação do Acervo A3.1", () => {
  ...
  test("reconcilia anexos canônicos, 16 documentos e B01 58 fotos + 1 SVG", async () => {
    ...
    expect(paginaAudio).toContain("Abrir transcrição pública deste documento");
    ...
    const divergente = anexos.map((a) =>
      a.arquivoId === correto.arquivoId ? { ...a, sha256: "0".repeat(64) } : a,
    );
  ...
  /*
    Aqui havia um caso de integração que consultava `vw_anexo_publico` com a
    credencial de DDL para provar que a projeção pública não expunha arquivo
    privado nem documento fora do gate. A prova era sobre a view, e a view saiu
    com o banco: o que o público lê agora é `acervo.json`, e os casos acima o
    conferem inteiro — 107 anexos, 16 documentos, nenhum campo privado.
  */
});
...
describe("contrato machine-readable do inventário público", () => {
  ...
  test("os anexos canônicos trazem arquivo_id e pagina_url, sem campo privado", async () => {
    ...
    expect(new Set(inventario.anexos.map((a) => a.arquivo_id)).size).toBe(107);
  ...
  test("nenhum MIME publicado cai no fallback técnico", async () => {
    const anexos = await listarAnexosPublicos();
